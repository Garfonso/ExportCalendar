...
/*global log: true, logGUI, logStatus: true, $L, Mojo, AppAssistant, PalmCall, config */
...
WelcomeAssistant.prototype.startExport = function (event) {
		...
		getResult = function (f) {
			if (f.result.finalResult) {
				...
				if (f.result.success) {
					...
					if (f.result.stats) {
						log("Exported " + f.result.stats.events + " in file " + config.filename + " with size " + f.result.stats.fileSize);
					}
				} else {
	...
	try {
		...
		log("Calling service.");
		future = PalmCall.call("palm://info.mobo.exportcalendar.service/", "doExport", config);
